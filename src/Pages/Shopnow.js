...
const ShopNow = () => {
    ...
    fetch("http://localhost:8000/api/products/")
      ...
      .then((data) => {
        ...
        const productsWithDiscount = data.map((product) => {
          const mrp = product.mrp || (product.amount * 1.4); // Assuming 40% markup if no MRP
          const discountPercent =
            mrp > product.amount
              ? Math.round(((mrp - product.amount) / mrp) * 100)
              : 0;
          const rating = product.rating || (Math.random() * 2 + 3).toFixed(1); // Random rating between 3.0-5.0
  ...
  useEffect(() => {
    ...
    // Apply gender/category filters from checkboxes
